...
use rtfm_core::app_state::{AppState, CreationMode, InputMode};
use rtfm_core::clipboard::ClipboardMode;

fn render_text_input_dialog(frame: &mut Frame, app_state: &AppState) {
    let title = match app_state.creation_mode {
        Some(CreationMode::Directory) => "New Directory",
        Some(CreationMode::File) => "New File",
        None => "Input", // Should not happen
    };
    let text = Paragraph::new(app_state.text_input.as_str())
...
pub fn render_main_layout(frame: &mut Frame, app_state: &AppState) {
    ...
    if app_state.input_mode == InputMode::TextInput {
        render_text_input_dialog(frame, app_state);
    }
}
